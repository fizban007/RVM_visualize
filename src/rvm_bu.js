...
var Config = function () {
  this.dipole_angle = 30.0;
  this.obs_angle = 90.0;
  this.pl_radius = 10.0;
  this.emission_h = 0.01;
  this.translation_d = 0.01;
...
gui.add(conf, "emission_h", 0.0, 1.0).listen();
gui.add(conf, "translation_d", 0.0, 1.0).listen();
...
const line = new WebglPlotBundle.WebglLine(white, numX);
...
function update() {
  ...
  for (let i = 0; i < line.numPoints; i++) {
    ...
    const xi = rad_obs;
    const delta = 0;
    const alpha = rad_dipole;
    const beta = 150/180 * Math.PI;

    const eq11a = (1 + eta - eps * Math.cos(delta) * Math.cos(xi)) * Math.sin(alpha) * Math.sin(beta + phi)
                  + eps * Math.sin(delta) * (Math.cos(alpha) * Math.cos(xi) * Math.sin(phi) - Math.sin(alpha) * Math.sin(beta) * Math.sin(xi));
    const eq11b = (1 + eta) * (Math.cos(alpha) * Math.sin(xi) - Math.sin(alpha) * Math.cos(xi) * Math.cos(beta +phi))
                      + eps * (Math.sin(alpha) * Math.cos(delta) * Math.cos(beta + phi) - Math.cos(alpha) * Math.sin(delta) * Math.cos(phi)); 
